...
여기서 배포란 단순히 코드를 원격 컴퓨터에 올리는 것이 아니라, 외부와 소통할 수 있도록 올바른 **네트워크 길(IP, Port, DNS, Proxy)을 열어주고 트래픽을 효율적으로 관리할 구조를 설계**하는 과정입니다.
...
Nginx를 WAS(Node.js) 앞단에 배치하여 **Reverse Proxy** 를 구성할 것입니다.

> 🔗 외부 사용자가 80(HTTP) 포트로 접근하면(네트워크 요청), Nginx가 이를 받아서 내부망 인터넷(Localhost)의 3000 포트를 사용하는 WAS로 다시 트래픽을 던져줍니다.

이는 **보안을 강화**하고 트래픽 부하를 **로드밸런싱** 시키는 전형적인 네트워크 인프라 설계이자 배포 전략입니다.

우분투 서버에 Nginx 설치 및 시작하기
...
> 🔗 PM2로 무중단/헬스체크 설정을 하는 것은, 결국 네트워크 상에서 **클라이언트와의 세션(Session)** 이나 **TCP 연결(L4)** 이 끊기지 않고 안정적으로 데이터를 전송(Transport)하기 위한 것입니다.
