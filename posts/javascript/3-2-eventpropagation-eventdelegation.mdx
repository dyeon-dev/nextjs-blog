...
이벤트 위임(event delegation)은 여러 개의 하위 DOM 요소에 각각 이벤트 핸들러를 등록하는 대신 **하나의 상위 DOM 요소에 이벤트 핸들러를 등록하는 방법**을 말한다.
...
추가된 엘리먼트에 역시 addEventListener를 통해서 이벤트 등록을 또 해줘야 한다. 이것도 꽤 불편한 일이다.
이처럼 불편한 문제를 해결하기 위해 이벤트 위임을 쓴다.
