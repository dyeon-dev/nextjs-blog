...
그 결과,
**디자인 시스템**을 도입함으로써 사용자보다는 "**개발자 경험**"을 향상시켜주었다면, **Concurrent Rendering**을 도입함으로써 개발자보다는 "**사용자 경험**"을 향상시켜주었다. _(사실 디자인 시스템이나 Concurrent나 모두 사용자, 개발자 경험을 향상했지만 굳이 따지자면 그렇다는 것이다.)_
...
모든 실패할 수 있는 경우에 try-catch문을 감싸지 않는 것처럼 Suspense를 일으키는 모든 컴포넌트에 Suspense나 ErrorBoundary를 붙여주기보다는, 적당한 부분 단위로 에러와 로딩 상태를 한번에 처리하면 된다.

이를 사용하기 위한 비동기 처리 라이브러리인 SWR이나 Tanstack-query에서는 `{suspense: true}` 옵션을 사용해주면 된다.
